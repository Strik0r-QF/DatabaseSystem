...
\begin{example}
    查询 “信息学院” 所有学生的姓名.
    \begin{sol}
    我们首先基于 student 和 department 关系的 dNo 属性将两个表
    做自然连接:
    \[ \mathrm{student}
    \Join_{\mathrm{student.dNo = department.dNo}} \mathrm{department}, \]
    然后筛选出学院名称为信息学院 (School of Infotmation Science, SIS)
    的元组,
    \[ \sigma_{\mathrm{dName = 'SIS'}}\left(
        \mathrm{student}
    \Join_{\mathrm{student.dNo = department.dNo}} \mathrm{department}
    \right), \]
    接下来将该关系投影到 sName 属性上得到查询结果, 因此
    ...
    \end{sol}
    \begin{cmt}
        由于在 \LaTeX 公式中输入汉字较为不便, 我们采用英文形式来给出关系代数
        表达式.
    \end{cmt}
\end{example}
